...
try:
    ...
    def _Radar_callback(radar_data):
        ...
        norn_velocity = detect_velocity / radar_velocity_range

        r = int(clamp(0.0, 1.0, 1.0 - norn_velocity) * 255.0)
        g = int(clamp(0.0, 1.0, 1.0 - abs(norn_velocity)) * 255.0)
        b = int(abs(clamp(-1.0, 0.0, -1.0 - norn_velocity)) * 255.0)
