...
val_list = list(object_id.value())

def generate_lidar_blueprint(blueprint_library):
	lidar_blueprint = blueprint_library.filter('sensor.lidar.ray_cast_semantic')
...
def semantic_lidar_data(point_cloud_data):
    ...
    for detection in point_cloud_data:
    	...
    	distance = math.sqrt((distance.point.x**2)+(distance_name_data.y**2)+(distance_name_data.z**2))
    	...
    	for i in object_id:
    		if distance_name_data['name'] == i and distance_name_data['distance'] > 5 and distance_name_data['distance'] < 7:
    			dropped_vehicle.apply_control(carla.VehicleControl(throttle = 0.31))

    		if distance_name_data['name'] == i and distance_name_data['distance'] > 3 and distance_name_data['distance'] < 6:
    			dropped_vehicle.apply_control(carla.VehicleControl(hand_brake = True))
    			dropped_vehicle.apply_control(carla.VehicleControl(throttle = 0.31))
    			dropped_vehicle.set_light_state(carla.VehicleLightState(carla.VehicleLightState.Brake | carla.VehicleLightState.LowBeam))
    			print(f"Object Near Vehicle: {i} and Distance: {distance_name_data['distance']}")
    			break
	...
	for detection in radar_data():
		...
		if distance_name_data['distance'] > 5 and distance_name_data['distance'] < 7:
		    ...
		    if distance_name_data['distance'] > 3 and distance_name_data['distance'] < 6:
			   dropped_vehicle.apply_control(carla.VehicleControl(hand_brake = True))
			   print('New distance, name and data', distance_name_data)
			   break
