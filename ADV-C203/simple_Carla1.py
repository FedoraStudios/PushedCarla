...
def main(arg):
    ...
    try:
        ...
        blueprint_library = world.get_blueprint_library() #gives the blueprint of the carla enviroment
        vehicle_bp = blueprint_library.filter(arg.filter)[0] #changes the vehicle model

        vehicle_transform = (world.get_map().get_spawn_points()[1]) #spawn location

        vehicle_transform.location.z += 3
        vehicle = world.sapwn_actor(vehicle_bp, vehicle_transform)
        vehicle.set_autopilot(arg.no_autopilot) #self driving
