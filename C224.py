...
def generate_lidar_blueprint(blueprint_library):
	lidar_blueprint = blueprint_library.find('sensor.lidar.ray_cast.semantic')
...
def semantic_lidar_data(point_cloud_data):
	...
	for detection in point_cloud_data:
		position = val_list.index(detetcion.object_tag)
		...
		for i in object_id:
			...
			if distance_name_data['name'] == i and distance_name_data['distance'] > 3 and distance_name_data['distance'] < 6:
				...
				print(f"Object Near Vehicle: {i} And Distance is: {distance_name_data['distance']}")
				break
