...
try:
    ...
    patrol_car = world.spawn_actor(police_car_blueprint, police_car_spawn_point)
    patrol_car.apply_control(carla.VehicleControl(throttle = 0.51))

    mustang_car_blueprint = get_blueprint_of_world.filter('mustang')[0]
    mustang_car_spawn_point = world.get_map().get_spawn_points()[4]
    mustang = world.spawn_actor(mustang_car_blueprint, mustang_car_spawn_point)
    mustang.apply_control(carla.VehicleControl(throttle = 0.51))
