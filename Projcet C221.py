    ...
    def _Radar_callback(radar_data):
        ...
        for detection in radar_data:
            ...
            if distance_name_data['distance'] > 3 and distance_name_data['distance'] < 6:
                dropped_vehicle.set_lightstate(carla.VehicleLightState(carla.VehicleLightState.Brake | carla.VehicleLightState.LeftBlinker | carla.VehicleLightState.LowBeam))
                
                dropped_vehicle.apply_control(carla.VehicleControl(throttle = 0.1))
                time.sleep(1)

                dropped_vehicle.apply_control(carla.VehicleControl(throttle = 0.5, steer = -0.75))
                time.sleep(2)

                dropped_vehicle.apply_control(carla.VehicleControl(throttle = 0.1, steer = 0.15))
                time.sleep(1)

                dropped_vehicle.apply_control(carla.VehicleControl(throttle = 0.1))
                time.sleep(1)

                dropped_vehcile.apply_control(carla.VehicleControl(hand_brake = True))
                time.sleep(2)
