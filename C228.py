...
try:
	...
	simulator_camera_location_rotation.location += spawn_point.get_foward_vector()*30
	...
	def throttle_vehicle():
		for vehicle_control_data in vehicle_control:
			dropped_vehicle.apply_control(carla.VehicleControl(throttle = float(vehicle_control_data[2]), brake = float(vehicle_control_data[0]), steer = float(vehicle_control_data[3])))
