...
try:
    sys.path.append(glob.glob('../carla/dist/carla-*%d.%d-%s.egg' %(
        sys.verion_info.major,
        sys.verion_info.minor,
...
def generate_lidar_blueprint(blueprint_library):
    lidar_blueprint = blueprint_library.find('sensor.lidar.ray_cast_semanatic')
...
def semanatic_lidar_data(point_cloud_data):
    distance_name_data = {}
    for detection in point_cloud_data:
        ...
        print('Distance and name data of surroundings: ', distance_name_data)
...
try: 
    ...
    spawn_points = world.get_map().get_spawn_points()[20]
    dropped_vehicle = world.spawn_actor(car_model, spawn_points)

    walkers_blueprint = random.choice(get_blueprint_of_world.filter('walker'))
    walkers_spawn_points = world.get_map().get_spawn_points()[15]
    dropped_walker = world.spawn_actor(walkers_blueprint, walkers_spawn_points)

    control_walker = carla.WalkerControl()
    carla_walker.speed = 0.9
    carla_walker.direction.y = 0
    carla_walker.direction.x = 1
    carla_walker.direction.z = 0
    dropped_walker.apply_control(control_walker)

    simulator_camera_location_rotation = carla.Transform(walkers_spawn_points.location, walkers_spawn_points.rotation)
    ...
    sensor.listen(lambda point_cloud_data: semanatic_lidar_data(point_cloud_data))
    
    car_control()
    time.sleep(1000)

finally:
    print('Destroying actrors.')
