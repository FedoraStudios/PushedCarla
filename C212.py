...
IM_WIDTH = 640
IM_HEIGHT = 480
#Show Camera Sensor Result with raw Data, Reshape Image show image with open cv
...
try:
	#Initialize carla client and carla world.
	client = carla.Client('127.0.0.1', 2000)
	client.set_timeout(20.0)
	world = client.get_world()
	get_blueprint_of_world = world.get_blueprint_library()

	#Definiton of car model, spawn point, address spawn point for car model, setting driving mode.
	car_model = get_blueprint_of_world.filter('model3')[0]
	spawn_point = random.choice(world.get_map().get_spawn_points())
	dropped_vehicle = world.spawn_actor(car_model, spawn_point)
	dropped_vehicle.set_autopilot(True)

	#Define carla simulated camera, location, rotation, forward focus, angle
	...
	#Define camera sensor, camera location, attachment of camera to vehicle, listen method.
	camera_sensor = camera(get_blueprint_of_world)
	sensor_camera_spawn_point = carla.Transform(carla.Location(x = 2.5, z = 0.7))
	sensor = world.spawn_actor(camera_sensor, sensor_camera_spawn_point, attached_to = dropped_vehicle)
	sensor.listen(lambda camera_data: image(camera_data))

	#Actor list for sensor and vehicle.
